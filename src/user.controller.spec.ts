...
describe('UserController', () => {
  ...
  describe('getById', () => {
    it('should return user if exists', async () => {
      expect(await userController.getById({ userId: EXISTING_ID }))
        .toStrictEqual({ id: EXISTING_ID, ...testUser });
    });

    it('should return undefined if user does not exist', async () => {
      expect(await userController.getById({ userId: NONEXISTING_ID })).toBe(undefined);
    });
  });

  describe('getMatchesById', () => {
    it('should return users if exist', async () => {
      expect(await userController.getMatchesById({ userId: EXISTING_ID }, undefined))
        .toStrictEqual([{id: ANOTHER_ID, ...testUser}]);
    });

    it('should return undefined if user does not exist', async () => {
      expect(await userController.getMatchesById({ userId: NONEXISTING_ID }, undefined)).toBe(undefined);
    });
  });
});
